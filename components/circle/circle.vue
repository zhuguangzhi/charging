<template>
	<view>
		<view  :style="bgStyle" class="shanxing">
		  ...
		  <view style="position: absolute;color: #000000;z-index: 999;display: flex;flex-direction: column;align-items: center;" class="">
			  <text>{{percent}}%</text>
			  <text style="font-size: 12px;">枪可用率</text>
		  </view>
		</view>
	</view>
</template>
	...
	export default {
		...
		data() {
			...
			sx2Style(){  
				...
				let width = this.width
				if((percents < 50 && percents != 0) || percents == 100){
				style =`background:${quancolor};clip: rect(0, ${width/2}px,${width}px, 0);width:${width}px;height:${width}px;transform: rotate(${percents < 50 ? ((360 * percents / 100) - 180):(360 * (percents - 100) / 100)}deg)`;
				}else{
				style =`-webkit-transform: rotate(180deg);background:${bgcolor};clip: rect(0, ${width/2}px,${width}px, 0);width:${width}px;height:${width}px;transform: rotate(${percents < 50 ? ((360 * percents / 100) - 180):(360 * (percents - 100) / 100)}deg);`;
				}
				return style
			}
		},methods:{
